...
Add to plan
[Edit](https://github.com/MicrosoftDocs/wsl/blob/main/WSL/build-custom-distro.md)
...
2. `/etc/wsl.conf`: A file containing global system settings that are specific to the user and control how the distribution is launched. [Learn more about WSL configuration files.](wsl-config).
...
The json file follows [the terminal profile json format](/en-us/windows/terminal/json-fragment-extensions#structure-of-the-json-files). Here's an example profile:
...
See [Advanced settings configuration in WSL](wsl-config) for all supported settings in `/etc/wsl.conf`.
...
To obtain a tar file of an existing Linux distribution, find guidance on how to export a docker container in [Import any Linux distribution to use with WSL](use-custom-distro).
...
Both registry values are strings (REG\_SZ) and are expected to be in URL format.
